...
def find_data_range(h5_file, start, end, resolution):
    with h5py.File(h5_file, "r") as h5_file:
        ...
        region_values = sparse_matrix[start:end, start:end].toarray()
...
def create_region_plot(h5_file, region, vmin, vmax, output_file, font_size):
...
def main():
    ...
    input_files = args.h5_files
    diff_h5_file = args.diff_h5_file
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for chrom, size in chrom_sizes.items():
            chrom_end_bin += size // resolution + 1

            region = {
                'chrom': chrom,
                'start': chrom_start_bin,
                'end': chrom_start_bin + ,
                'name': chrom
            }
            ...
            for idx, input_file in enumerate(input_files):
                max_values.extend(find_data_range(input_file, chrom_start_bin, chrom_end_bin, resolution))
            vmax = max(max_values)
            ...
            for idx, input_file in enumerate(input_files):
                ...
                create_region_plot(input_file, region, 0, vmax, output_file, args.font_size)
            ...
            chrom_start_bin = chrom_end_bin
        # Combine all pages into final PDF
        merger = PdfFileMerger()
        for page_file in final_pages:
            merger.append(page_file)

        inputdir, inputbase = os.path.split(diff_h5_file)
        base_name = inputbase.rsplit('_', 1)[0]
        outfile = os.path.join(inputdir, f"{base_name}_{chrom}_{resolution}.pdf")
