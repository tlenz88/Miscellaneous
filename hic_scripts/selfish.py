...
def parse_args(args):
	...
	parser.add_argument("-f1",
						"--file1",
						dest="f1_path",
						help="Contact map for conditional - Juicebox type input (.hic)",
						required=False)
	parser.add_argument("-f2",
						"--file2",
						dest="f2_path",
						help="Contact map for control - Juicebox type input (.hic)",
						required=False)
	...
	parser.add_argument("-bed1",
						"--bed1",
						dest="bed1",
						help="BED file 1 for for conditional - HiC-Pro type input",
						default="",
						required=False)
	parser.add_argument("-bed2",
						"--bed2",
						dest="bed2",
						help="BED file 2 for for control - HiC-Pro type input",
						default="",
						required=False)
	parser.add_argument("-m1",
						"--matrix1",
						dest="mat1",
						help="MATRIX file for conditional - HiC-Pro type input (.matrix)",
						default="",
						required=False)
	parser.add_argument("-m2",
						"--matrix2",
						dest="mat2",
						help="MATRIX file for control - HiC-Pro type input (.matrix)",
						default="",
						required=False)
	parser.add_argument("-b1",
						"--biases1",
						dest="biasfile1",
						help="RECOMMENDED: biases calculated by ICE or KR norm for each locus for contact map 1 are read from BIASFILE",
						required=False)
	parser.add_argument("-b2",
						"--biases2",
						dest="biasfile2",
						help="RECOMMENDED: biases calculated by ICE or KR norm for each locus for contact map 2 are read from BIASFILE",
						required=False)
	...
	parser.add_argument("-p",
						"--plot",
						dest="plot",
						help="OPTIONAL: use this flag for generating plots. DEFAULT is False.",
						default=False,
						required=False)
	...
	#					action="version",
	#					version="Selfish {}".format(__version__))
	return parser.parse_args()

def read_map_pd(path, res, bias, ch):
	"""
...
def DCI(f1,
	...
	#	print("Interchromosomal analysis is only supported for .hic and .cool input formats.")
	#	raise FileNotFoundError
	...
	if type(f1) == str and type(f2) == str:
		...
		if bed1 != '' and f1 != '':
			if chromosome == 'n':
				...
				raise FileNotFoundError
			a = readBEDMAT(bed1, f1, res, chromosome, chromosome2, biasDict1)
		elif f1.endswith('.hic'):
			if chromosome == 'n':
				print("You need to specify a chromosome for hic files.")
				raise FileNotFoundError
			a = readHiCFile(f1, chromosome, chromosome2, res, distance_filter // res, norm_method)
		...
		if bed2 != '' and f2 != '':
			if chromosome == 'n':
				print("You need to specify a chromosome for matrix files.")
				raise FileNotFoundError
			b = readBEDMAT(bed2, f2, res, chromosome, chromosome2, biasDict2)
		elif f2.endswith('.hic'):
			if chromosome == 'n':
				print("You need to specify a chromosome for hic files.")
				raise FileNotFoundError
			b = readHiCFile(f2, chromosome, chromosome2, res, distance_filter // res, norm_method)
	...
	elif type(f1) == np.ndarray and type(f2) == np.ndarray and f1.shape[0] == f1.shape[1] and f2.shape[0] == f2.shape[1]:
		a = f1.copy()
		f1 = "Map 1"
		b = f2.copy()
		f2 = "Map 2"
	...
	changes_array[non_zero_indices] = np.log2(np.divide(a + 1, b + 1))[non_zero_indices]
	...
	if st:
		...
		for i in range(len(x)):
			s = math.ceil(scale[i])
			c1 = np.sum(os[x[i]-s:x[i]+s+1, y[i]-s:y[i]+s+1]) / ((2*s+1)**2)
			s = 2*s
			c2 = np.sum(os[x[i]-s:x[i]+s+1, y[i]-s:y[i]+s+1]) / ((2*s+1)**2)
			if c1 < st or c2 < 0.6:
				nonsparse[i] = False
		x = x[nonsparse]
		y = y[nonsparse]
...
def readHiCFile(f, chr, chr2, res, distance, norm_method):
	...
	"""
	if not norm_method:
		result = straw.straw('observed', 'KR', f, str(chr), str(chr2), 'BP', res)
	else:
		result = straw.straw('observed', str(norm_method), f, str(chr), str(chr2), 'BP', res)
	binX = []
	binY = []
	counts = []
	for i in range(len(result)):
		binX.append(result[i].binX // res)
		binY.append(result[i].binY // res)
		counts.append(result[i].counts)
	df = pd.DataFrame(list(zip(binX, binY, counts)), columns=['x', 'y', 'val'])
	n = max(max(binX), max(binY)) + 1
	o = np.zeros((n, n))
	for i in df.itertuples(n):
		o[i.x, i.y] = i.val
		o[i.y, i.x] = i.val
	np.nan_to_num(o, copy=False, nan=0, posinf=0, neginf=0)
	return o
...
def readBEDMAT(bed, mat, res, chr, chr2, bias):
	"""
		...
		with open(bed) as bf:
			for line in bf:
				...
				if l[0] == chr:
					d[int(l[3])] = toNearest(int(l[2]), res) // res
				if l[0] == chr2:
					d2[int(l[3])] = toNearest(int(l[2]), res) // res
		n = max(d.values()) + 1
		n2 = max(d2.values()) + 1
		o = np.zeros((n,n2))
		with open(mat) as mf:
			for line in mf:
				l = line.strip().split('\t')
				if int(l[0]) in d and int(l[1]) in d2:
					val = float(l[2])
					if bias:
						val /= bias.get(int(l[1]), 1)
						val /= bias.get(int(l[0]), 1)
					o[d[int(l[0])], d2[int(l[1])]] = val
				elif int(l[1]) in d and int(l[0]) in d2:
					val = float(l[2])
					if bias:
						val /= bias.get(int(l[1]), 1)
						val /= bias.get(int(l[0]), 1)
					o[d[int(l[1])], d2[int(l[0])]] = val
	np.nan_to_num(o, copy=False, nan=0, posinf=0, neginf=0)
	return o
	...
	if tsvout:
		if args.st:
			with open(args.outdir, 'w') as outfile:
				outfile.write('chr1\tbin1\tchr2\tbin2\tq_val\tlog2FC\n')
				for i in range(len(x)):
					_x, _y = x[i], y[i]
					outfile.write(f'{args.chromosome}\t{_x * res}\t{args.chromosome2}\t{_y * res}\t{q_val[i]}\t{changes_array[i]}\n')
		else:
			...
			with open(args.outdir, 'w') as outfile:
				...
				for i in indices:
					_x, _y = i[0], i[1]
					outfile.write(f'{args.chromosome}\t{_x * res}\t{args.chromosome2}\t{_y * res}\t{o[_x,_y]}\t{changes_array[_x,_y]}\n')
	else:
